...
for (i in seq_along(files.v)) {
  ...
  a <- which(str_detect(colnames(working.df), "_Is_"))    #This does not coincide with step_2 starting at line 469!!
  b <- which(str_detect(colnames(working.df), "_Are_"))   #change to "_ISA_"
  
  c <- c( a, b)
  working.df <- working.df[, c]
  
  working.df <- working.df[sample(1:nrow(working.df), 1500), ] #20000 -> 1500.
...
for (i in 1:100) {
  ...
  testing.index.v <- sample (seq (1, nrow(final.tib)), 600, prob=final.tib$probs)
