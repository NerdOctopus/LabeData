...
for (i in seq_along(files.v)) { # a loop to open csv files and save them as .RDS files; creating a separate output directory is recommended
  ...
  working.df <-  read.csv(file = files.v[i])
...
working.df <-  read.csv(file = files.v[13])
op <- gsub(".csv", ".RDS", files.v[13])
saveRDS(working.df, file = op )
...
for (i in seq_along(files.v)) { # loop to make separate col for each category of morpho-syntactic data
  ...
  fp <- file.path("C:\\LabeWork\\rds\\parsed_expanded", files.v[i]) # create file path for saving
...
setwd("C:\\LabeWork\\rds\\parsed_expanded")
files.v <- dir(pattern = ".RDS") # load filenames into vector
...
setwd("C:\\LabeWork\\rds\\parsed_expanded\\variables")
...
# these data are stored at C:\Users\rgorm\Documents\non-AGLDT_trees\selected_var_val_pairs- for me, C:\LabeWork\rds\parsed_expanded\variables
...
for (i in seq_along(files.v)) {
  ...
  file.path("C:", "LabeWork", "rds", "parsed_expanded", "binary_valued")
  
  file_name <- paste0(file_name,   "_binary_valued",  ".RDS") # add file type to file name for saving
  
  # ! make sub directory in current working directory called "binary_valued." Otherwise the following lines will not work!
  fp <- file.path("C:", "LabeWork", "rds", "parsed_expanded", "binary_valued", file_name) # file path for save
